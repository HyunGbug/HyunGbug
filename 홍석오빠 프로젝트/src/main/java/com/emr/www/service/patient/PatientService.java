...
//	}
	...
	public void registerPatientVisit(PatientVisitDTO patientVisitDTO) {
	    try {
	        ...
	        DoctorEntity doctor = doctorRepository.findBySecurityNum(patientVisitDTO.getDoctorSecurityNum())
	                .orElseThrow(() -> new IllegalArgumentException("유효하지 않은 의사 정보입니다."));

	        // 3. 간호사의 주민등록번호로 간호사 정보 조회
	        NurseEntity nurse = nurseRepository.findBySecurityNum(patientVisitDTO.getNurseSecurityNum())
	                .orElseThrow(() -> new IllegalArgumentException("유효하지 않은 간호사 정보입니다."));

	        // 4. PatientVisit 엔티티 생성 및 데이터 설정
	        PatientVisitEntity visitEntity = new PatientVisitEntity();
	        visitEntity.setPatientNo(patient.getNo());
	        visitEntity.setDoctorNo(doctor.getNo());
	        visitEntity.setNurseNo(nurse.getNo());
